// RaytracerSIMD.cpp : Defines the entry point for the console application.
//
#include "stdafx.h"
...
int main(int argc, char **argv)
{
	...
	// Create data file
	ofstream data("samplesSIMD.csv", ofstream::out);
				...
				for (size_t x = 0; x < dimension; ++x)
				{
					for (size_t sy = 0, i = (dimension - y - 1) * dimension + x; sy < 2; ++sy)
					{
						for (size_t sx = 0; sx < 2; ++sx)
						{
							...
							pixels[i] = pixels[i] + vec(clamp(r.get_x(), 0.0, 1.0), clamp(r.get_y(), 0.0, 1.0), clamp(r.get_z(), 0.0, 1.0)) * 0.25;
						}
					}
				}
			}
